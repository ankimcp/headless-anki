...
Each variant's Dockerfile takes a `BASE_TAG` (or `X11_TAG`) ARG defaulting to `ghcr.io/ankimcp/headless-anki:base-v1.5.0`. CI overrides this; locally it uses the default.
...
# Build with addons
cd x11-vnc-addons
docker build --build-arg ADDON_IDS="2055492159" -t headless-anki:addons-v1.5.0 .
docker compose up
```

Version tags are hardcoded to `v1.5.0` in `run.sh` and `docker-compose.yaml` files. When bumping versions, update these across all variant directories.
